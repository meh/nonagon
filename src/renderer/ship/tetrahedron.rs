...
impl<'a> Tetrahedron<'a> {
	pub fn new<'b>(display: &'b Display) -> Tetrahedron<'b> {
		#[inline(always)]
		...
		Tetrahedron {
			...
			vertices: VertexBuffer::new(display, vec![
				...
				// right
				Vertex { position: [ 0.0,  1.0,  0.0], texture: coordinates(3, 0.5, 0.0) },
				Vertex { position: [ 1.0, -1.0,  1.0], texture: coordinates(3, 0.0, 1.0) },
				Vertex { position: [ 0.0, -1.0, -1.0], texture: coordinates(3, 1.0, 1.0) },

				// bottom
				Vertex { position: [-1.0, -1.0,  1.0], texture: coordinates(4, 0.5, 0.0) },
				Vertex { position: [ 0.0, -1.0, -1.0], texture: coordinates(4, 0.0, 1.0) },
	...
	pub fn render<T: Surface>(&mut self, target: &mut T, support: &Support, state: &game::Ship) {
		let mvp = support.scene().to_mat() *
			...
			support.scene().scale(25.0 * state.scale);
